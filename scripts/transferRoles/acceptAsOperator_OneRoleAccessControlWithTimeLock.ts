...
async function main() {
    ...
    const tx = await OneRoleAccessControlWithTimeLock.connect(moreSecuredOperator).acceptAsMoreSecuredOperator()
    console.log(`acceptAsMoreSecuredOperator tx sent: ${tx.hash}`)
    await tx.wait()
}
